class StreakBadgeJob < ApplicationJob
  ...
  def perform(message)
    ...
    60.times do |num|
      status = Like.where(created_at: (@today_date - num).beginning_of_day..(@today_date - num).end_of_day, is_judged: true, user_id: message.user_id).where.not(post_id: nil).present?
      @result << status
      case num
      when 30
