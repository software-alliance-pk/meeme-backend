class FollowerBadgeJob < ApplicationJob
  ...
  def perform(message)
    ...
    if message.user.followers.follower_added.count == 100000
    ...
    elsif message.user.followers.follower_added.count == 5000
    ...
    if current_user.followers.follower_added.count == 100000
      @badge = Badge.find_by(title: "I Love My Community Badge")
    ...
    elsif current_user.followers.follower_added.count == 5000
      @badge = Badge.find_by(title: "The Part Of The Part of Crowd Badge")
      @check = UserBadge.find_by(user_id: current_user.id, badge_id: @badge.id)
      if @check.present?
      else
    ...
    elsif current_user.followers.follower_added.count == 100
      @badge = Badge.find_by(title: "Follower Badge")
      @check = UserBadge.find_by(user_id: current_user.id, badge_id: @badge.id)
      if @check.present?
      else
