...
json.tournament_days @difference
...
json.judged_posts do
  json.array! (@difference).times do |num|
    ...
    json.status   @posts_judged.where(created_at: (@tournament_start_date + num).beginning_of_day..(@tournament_start_date + num).end_of_day).count>99?  true: false
