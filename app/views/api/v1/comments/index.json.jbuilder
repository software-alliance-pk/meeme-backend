json.comments do
  json.(@comments) do |comment|
    ...
    json.user_image comment.user.profile_image.attached? ? CloudfrontUrlService.new(comment.user.profile_image).cloudfront_url : ''
    json.user_comment_like_status comment.likes.where(user_id: @current_user.id).present? ? true : false
    json.comment_image comment.comment_image.attached? ? CloudfrontUrlService.new(comment.comment_image).cloudfront_url : ''
    ...
    json.child_comment comment.comments do |child_comment|
      ...
      json.comment_image child_comment.comment_image.attached? ? CloudfrontUrlService.new(child_comment.comment_image).cloudfront_url : ''
