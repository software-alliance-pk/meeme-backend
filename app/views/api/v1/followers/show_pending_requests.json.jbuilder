...
json.pending_followers @user_followers.each do|follower|
  ...
  json.user_image User.find(follower.user_id).profile_image.attached? ? User.find(follower.user_id).profile_image.blob.variant(resize_to_limit: [512, 512],quality:50).processed.url : ''
  ...
  json.follower_image User.find(follower.follower_user_id).profile_image.attached? ? User.find(follower.follower_user_id).profile_image.blob.variant(resize_to_limit: [512, 512],quality:50).processed.url : ''
